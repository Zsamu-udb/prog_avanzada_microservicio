// js/modules/clientes/ClienteModule.js
...
export class ClienteModule {
  ...
  async loadClientes() {
    ...
    this.clientes = data || [];
    this.renderTable();
    if (this.onChange) this.onChange(); // notifica cambios a reservas
  }
  ...
  renderTable() {
    ...
    if (this.clientes.length === 0) {
      ...
      tr.innerHTML =
        '<td colspan="5" class="table__empty">Sin clientes registrados</td>';
    ...
    this.clientes.forEach((c) => {
      ...
      tr
        ...
        .addEventListener("click", () => this.showHistorial(c));
      tr
        .querySelector('[data-action="edit"]')
        .addEventListener("click", () => this.fillForm(c));
      tr
        .querySelector('[data-action="delete"]')
        .addEventListener("click", () => this.delete(c.id));
  ...
  async onSubmit(e) {
    ...
    try {
      if (id) {
        ...
        this.modal.show("Cliente actualizado correctamente");
      } else {
        await this.service.create(data);
        this.modal.show("Cliente creado correctamente");
      }

      this.form.reset();
      this.form.id.value = "";
      await this.loadClientes(); // onChange se dispara dentro
    } catch (err) {
  ...
  async delete(id) {
    ...
    try {
      ...
      this.modal.show("Cliente borrado correctamente");
      await this.loadClientes(); // onChange se dispara dentro
    } catch (err) {
      this.modal.show(err.message, "Error");
    }
  ...
  async showHistorial(cliente) {
    try {
      ...
      if (!reservas.length) {
        this.modal.show(
          `El cliente "${cliente.nombre}" no tiene reservas.`,
          "Historial"
        );
        return;
      }
      ...
      const lineas = reservas
        .map(
          (r) =>
            `• ${r.fecha_inicio} → ${r.fecha_fin}  [${r.estado}]  Vehículo: ${
              r.vehiculo?.marca
